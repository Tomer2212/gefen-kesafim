...
_TABLE = "meeting_booking_drafts"


def _create_draft(org_id: str, created_by: str, criteria_text: str, months: list[str], schools: list[dict]) -> dict:
...
def _update_draft(draft_id: str, org_id: str, patch: dict) -> dict:
    """Update a draft, scoped to org_id. Raises on failure (unlike the fire-and-forget
    run_states pattern) — this is on the critical path of a paid action (sending emails),
    a silently-dropped update here could let a partially-resolved draft be confirmed."""
    for attempt in range(2):
        try:
            db = get_admin_client()
            row = (
