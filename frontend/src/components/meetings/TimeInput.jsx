...
export function TimeInput({ id, value, onChange, ariaLabel, hasError, errorMessage }) {
  ...
  return (
    <input id={id} type="text" inputMode="numeric" maxLength={5}
      className={`w-full bg-transparent text-sm text-right hover:bg-slate-100 hover:rounded focus:bg-white focus:rounded py-0.5 px-0 focus:px-1 transition-all ${
        hasError
          ? "border border-red-500 ring-1 ring-red-500 rounded text-red-700"
          : "border-0 outline-none focus:ring-1 focus:ring-blue-300 text-slate-700"
      }`}
      placeholder=""
      value={value} onChange={handleChange} onBlur={handleBlur}
      onKeyDown={e => { if (e.key === "Enter") e.currentTarget.blur(); }}
      aria-label={ariaLabel} aria-invalid={hasError || undefined}
